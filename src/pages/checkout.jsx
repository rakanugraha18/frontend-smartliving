...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      try {
        ...
        // Ambil data pesanan dari respons
        const order = response.data.order;

        // Periksa status dan hanya simpan pesanan dengan status 'pending'
        if (order.status === "pending") {
          setOrderData(order);
        } else {
          // Jika status bukan 'pending', atur state menjadi null atau kosong
          setOrderData(null);
          console.log("Pesanan tidak memiliki status 'pending'.");
        }
      } catch (error) {
  ...
  const isFormValid = () => {
    return selectedAddressId && paymentMethod && selectedBank && isTermsChecked;
  };
                ...
                <div className="items-start p-4">
                  <label className="inline-flex items-center">
                    ...
                    <span className="ml-2 text-sm">
                      Saya mengijinkan SmarLiving untuk mengirimkan newsletter
...
//         } else {
//           console.log("Tidak ada pesanan untuk user:", userId);
//         }
//       } catch (error) {
...
//         const response = await axios.get(
//           `http://localhost:3000/api/order/${userId}/${selectedOrderId}`,
//           {
//             headers: {
//               Authorization: `Bearer ${token}`,
//             },
...
//       await axios
//         .put(
//           `http://localhost:3000/api/order/${selectedOrderId}`,
//           requestData,
//           {
//             headers: {
//               Authorization: `Bearer ${token}`,
...
//       {/* Order Selection */}
//       <div className="flex justify-center items-center bg-[#EBEEF3] h-[51px] mb-4">
//         <h2 className="text-xl font-semibold">PILIH PESANAN</h2>
//       </div>
//       <div className="mb-4">
//         {orders.map((order) => (
...
//                 selectedOrderId === order.id ? "bg-gray-200" : ""
//               }`}
//             >
